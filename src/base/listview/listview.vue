<template>
    <scroll class="listview" :data='data' ref="listview" @scroll="scroll" :listen-scroll="listenScroll" :probeType='probeType'>
        <ul>
...
export default {
  ...
  computed: {
    shortcutList() {
      return this.data.map(group => {
        return group.title.substr(0, 1);//针对"热门"切割
      });
    },
    fixedTitle() {
      if (this.scrollY > 0) {
  ...
  methods: {
    onShortcutTouchStart(e) {
      let anchorIndex = getData(e.target, "index");//获取data-index的值
    ...
    onShortcutTouchMove(e) {
      ...
      let delta = ((this.touchs.y2 - this.touchs.y1) / ANCHOR_HEIGHT) | 0;
      let anchorIndex = parseInt(this.touchs.anchorIndex) + delta;
      this._scrollTo(anchorIndex);
    },
    scroll(pos) {
      this.scrollY = pos.y;
  ...
  watch: {
    data() {
      setTimeout(() => {
        this._calculateHeight();//相当于nextTick,兼容性好
      }, 20);
    },
    scrollY(newY) {
      ...
      for (let i = 0; i < listHeight.length - 1; i++) {
        ...
        if (!height2 || (-newY >= height1 && -newY < height2)) {
          this.currentIndex = i;
          this.diff = height2 + newY; //newY是负值，所以求的是顶部和title间的差距
          // console.log(i)
          return;
        }
      }
